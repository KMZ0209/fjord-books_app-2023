...
class ReportMentionTest < ApplicationSystemTestCase
  ...
  test 'carolの日報の本文にbobの日報URLが含まれている場合bobの日報に「この日報に言及している日報：」とcarolの日報タイトルがリンクとして表示される' do
    click_on '日報'
  ...
  test 'carolの日報を更新してbobの日報内の言及も更新する' do
    click_on '日報'
    assert_selector 'h1', text: '日報の一覧'
  ...
  test 'carolの日報内でbobの日報への言及を削除する' do
    click_on '日報'
    assert_selector 'h1', text: '日報の一覧'
    within('.index-item', text: 'タイトル: 言及テスト') do
      click_link('この日報を表示')
